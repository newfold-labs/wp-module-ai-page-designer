...
**Stage 2's v1 catalogue is complete (10/10 archetypes), the live page-plan pipeline is proven and manually confirmed in the browser, page-plan is now on by default, and the preview/frontend motion-CSS drift is fixed** (see the Stage 2 section and "Unified preview + frontend motion CSS" above). Next: multi-variant support per archetype, and "editing on the plan" once the plan-based flow has more real usage to justify it.

...
**Plugin-dependency fallback (2026-08-12).** Asked for a contact form, the model reaches for a plugin it saw in training — and there is no plugin. It does this two ways, and live testing hit both within an hour of each other:

1. **As a block** — `<!-- wp:forminator/contact-form {"id":42} /-->`. A self-closing block whose type isn't registered renders *nothing*: no plugin, no output, no error. The section's heading showed and the form silently didn't.
2. **As a shortcode** — `[contact-form-7 id="0" title="Contact form"]`. An unregistered shortcode isn't stripped, it's printed, so the visitor reads the raw tag as body text. Note the invented `id="0"`: the model can't know a real form ID, so this is broken even *with* the plugin installed — which is why a form shortcode with an empty/`0` id counts as unrenderable regardless of registration.

One rule covers both (`UnrenderableContentFallback`, runs second, right after `RepairDelimiters`) — same defect class, same substitution, one tree walk. Form-shaped content becomes the theme-styled `core/html` form `BookingForm` renders (its `render_form_block()` is now public — one definition of "what a form looks like"). The two halves deliberately differ on *non*-form content: an unrenderable **block** is dropped (a block delimiter is never prose), while an unrecognised **shortcode** is left alone (`[see below]` is prose; deleting a visible sentence is worse than one stray tag).

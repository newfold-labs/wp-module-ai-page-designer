...
**Catalogue expanded to 13 archetypes + modern card variants (2026-07-05, user-verified live).** Three new archetypes: `galleryGrid` (rows of 3 rounded images, ≤6, per-image Unsplash queries), `teamGrid` (≤4 people cards: circular avatar, name, role, bio), `processSteps` (3-4 numbered steps; the number badge is a real `core/paragraph` with accent color attrs — selectable/recolorable, not raw HTML). Three modern DEFAULT variants: `featureGrid: floating-cards`, `testimonials: cards`, `statsBar: stat-cards` (old flat looks reachable via explicit `variant`). New shared helper `RendersMarkup::card_slug_for_section()` — the quiet muted-light/light card swatch (vs. `contrasting_slug()`'s loud accent, right for CTAs, garish for grids), null (transparent card) when nothing differs from the section bg, so cards never collide by construction. Page-plan prompt steers: galleryGrid for visual businesses, teamGrid for about/people, processSteps for how-it-works flows. 163 tests (was 135) incl. a composed 13-archetype page validating clean + conform-idempotent.

**Plugin block fallback (2026-08-12).** "Add a contact form" to a freeform-generated page came back as headings with nothing under them: the model emitted `<!-- wp:forminator/contact-form {"id":42} /-->`, and a self-closing block whose type isn't registered renders *nothing* — no plugin, no output, no error. New rule `UnsupportedBlockFallback` (runs second, right after `RepairDelimiters`) replaces any block that is both unregistered **and** renders nothing: form-shaped ones become the same theme-styled `core/html` form `BookingForm` renders (now its `render_form_block()` is public — one definition of "what a form looks like"), everything else is dropped. Two guards keep it from eating real markup: a block with saved HTML is never touched (JS-only-registered plugin blocks still render), and if `core/paragraph` itself looks unregistered the registry is treated as not-yet-populated and only the namespace is trusted, so no call site can blank a page. Also catches experimental `core/form`, which fails identically on a stock install. Note the ordering lesson: the rule was first placed *after* `SanitizeCss`, which left the substituted form's inline styles unsanitized until the next conform — i.e. until save, reintroducing exactly the saved≠previewed drift the harness exists to prevent. Anything that *injects* markup must run before the rules that clean markup. 281 tests green.
...
| `UnsupportedBlockFallback` | swap a block this site can't render for one it can | `unsupported_block:<name>` | done 2026-08-12 |
...
**One canonical motion vocabulary** (classes `fade-in`, `slide-up`, `scale-in`, `bounce-in`, `card-hover-lift`, `pulse-hover`, `glow-hover`, `fade-in-delay-1/2/3` + `data-aos`/`data-aos-delay`) is the single source of truth, generated by `AIPageDesigner::get_motion_css()` and consumed by both the preview and the front-end — see "Unified preview + frontend motion CSS" above, which closed the exact `pulse-hover`-style drift this section originally called out as a risk. **Per-archetype curated motion landed 2026-07-05 (`107126e`):** every section emits `data-aos` (one-shot scroll-triggered fade-up via the IntersectionObserver both surfaces already run) and every floating card emits `card-hover-lift` (hover-gated) — both from this vocabulary, emitted structurally by the shared `RendersMarkup` helpers so no archetype hand-picks classes. Two hard-won integration notes: the preview's `nfdSetContent` finalize path must render content BEFORE `_enableAnimations()` attaches the observer (observing the about-to-be-replaced nodes leaves fresh `data-aos` sections permanently at base `opacity:0` — a fully blank page); and the progressive-reveal hide rule needs a `[data-nfd-streaming]`-qualified variant to out-cascade the srcdoc's streaming `[data-aos]{opacity:1 !important}` guard.

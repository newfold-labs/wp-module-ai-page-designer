...
- COLOR AND STYLE CHANGES: Use a two-layer contract:
   ...
   7. COLOR CONSISTENCY VERIFICATION: After applying any color changes, verify that every text element OUTSIDE dark-background containers received the requested color. Text INSIDE dark-background containers must remain white or light — never dark.
   8. NESTED ELEMENTS: Pay special attention to text inside nested structures like columns, groups, covers, media-text blocks, and buttons. Text can be deeply nested — traverse ALL levels of nesting when applying colors, but always skip dark-background container descendants.
